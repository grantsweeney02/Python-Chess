...
# a cache to avoid loading check_point2 many time
_known_images = {}


def _image(key, flip=False, w=0, h=0, angle=0):
    """A method for loading check_point2, caching them, and flipping them"""
...
def _image_from_url(url):
    '''a method for loading check_point2 from urls by first saving them locally'''
...
def _image_from_file(filename):
    '''a method for loading check_point2 from files'''
...
def _get_image(thing):
    '''a method for loading check_point2 from cache, then file, then url'''
...
def load_sprite_sheet(url_or_filename, rows, columns):
    '''Loads a sprite sheet. Assumes the sheet has rows-by-columns evenly-spaced check_point2 and returns a list of those check_point2.'''
